...
export default function NewTransactionPage() {
  ...
  return (
    <div className="mx-auto max-w-xl">
      <PageHeader title="New transaction" description="Record income, an expense, or a transfer." />

      {!accountsLoading && accountList.length === 0 ? (
        <EmptyState
          icon={Wallet}
            ...
            <form action={action} className="space-y-5">
              ...
              {type !== "EXPENSE" && (
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{type === "TRANSFER" ? "To account" : "Account"}</FormLabel>
                      <FormControl>
                        <AccountSelect
                          accounts={accountList}
                          value={field.value}
